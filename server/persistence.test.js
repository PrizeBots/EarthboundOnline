 ...
 *
 * The store contract is async-capable (Supabase is async; SQLite is sync). This
 * test is written backend-agnostically: every store/join/save touch is awaited.
 * `recv` returns the message handler's promise; `s.close()` is followed by
 * `host.flushSaves()` so a queued save-back is durable before we read it.
 *
...
class FakeSocket {
  ...
  // Returns the handler's promise (the join path awaits the store), so callers
  // can `await s.recv(...)` for messages that read/write persistence.
  recv(obj) {
    return this.handlers.message(JSON.stringify(obj));
  }
  close() {
...
async function main() {
  ...
  const account = await store.createAccount({ username: 'Hero', passwordHash: 'h', now: T });
  await store.createSession({ token: 'sess-ok', accountId: account.id, now: NOW, ttlMs: YEAR });
  ...
  const other = await store.createAccount({ username: 'Rando', passwordHash: 'h', now: T });
  await store.createSession({ token: 'sess-other', accountId: other.id, now: NOW, ttlMs: YEAR });

  // ---- join by token loads the derived build ----

  const s1 = new FakeSocket();
  host.handleConnection(s1);
  await s1.recv({ type: 'join', sessionToken: 'sess-ok', characterId: character.id });
  const pid = s1.last('welcome').playerId;

  await check('join-by-token sends a welcome with self + stats', () => {
  ...
  await check('combat stats are derived from the saved allocation', () => {
  ...
  await check('earning EXP then disconnecting persists level + exp + banked points', async () => {
    ...
    const saved = (await store.getCharacter(character.id)).save;
  ...
  await check('spend_points rejects cheats and applies a valid spend; persists', async () => {
    const s = new FakeSocket();
    host.handleConnection(s);
    await s.recv({ type: 'join', sessionToken: 'sess-ok', characterId: character.id });
  ...
  await check('setting flags persists them; rejoin restores via welcome.flags', async () => {
    const s = new FakeSocket();
    host.handleConnection(s);
    await s.recv({ type: 'join', sessionToken: 'sess-ok', characterId: character.id });
    const id = s.last('welcome').playerId;

    await s.recv({ type: 'set_flag', id: 900001 });
    await s.recv({ type: 'set_flag', id: 900002 });
    await s.recv({ type: 'set_flag', id: 900001 }); // duplicate — no-op
    await s.recv({ type: 'clear_flag', id: 900002 }); // toggled back off
    ...
    await s2.recv({ type: 'join', sessionToken: 'sess-ok', characterId: character.id });
    assert.deepStrictEqual(s2.last('welcome').flags, [900001], 'welcome restores saved flags');

    await s2.recv({ type: 'clear_all_flags' });
    assert.strictEqual(host.flags.get(s2.last('welcome').playerId).size, 0, 'reset wipes flags');
    s2.close();
    await host.flushSaves();
    assert.deepStrictEqual(
      (await store.getCharacter(character.id)).save.flags,
      [],
      'reset persists empty'
    );
  });
  ...
  await check(
    'PK + remaining in-game lock persist; reload resumes (offline not counted)',
    async () => {
      const s = new FakeSocket();
      host.handleConnection(s);
      await s.recv({ type: 'join', sessionToken: 'sess-ok', characterId: character.id });
      ...
      await s.recv({ type: 'set_pk', on: true });
  ...
  await check('rejoining restores the saved level', async () => {
    const s2 = new FakeSocket();
    host.handleConnection(s2);
    await s2.recv({ type: 'join', sessionToken: 'sess-ok', characterId: character.id });
    const w = s2.last('welcome');
  ...
  await check('a bad session token is rejected with join_error', async () => {
    const s = new FakeSocket();
    host.handleConnection(s);
    await s.recv({ type: 'join', sessionToken: 'nope', characterId: character.id });
    assert(s.last('join_error'), 'expected join_error');
    assert(!s.last('welcome'), 'must not welcome an invalid session');
  });

  await check("another account can't load a character it doesn't own", async () => {
    const s = new FakeSocket();
    host.handleConnection(s);
    await s.recv({ type: 'join', sessionToken: 'sess-other', characterId: character.id });
    assert(s.last('join_error'), 'expected join_error for non-owned character');
  });

  await check('anonymous join (no token) still works for the dev/char-select path', async () => {
    const s = new FakeSocket();
    host.handleConnection(s);
    await s.recv({ type: 'join', name: 'Guest', spriteGroupId: 2 });
