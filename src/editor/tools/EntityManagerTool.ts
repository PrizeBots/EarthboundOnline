...
import { buildInflictEditor, InflictEntry } from '../components/InflictEditor';
import { buildAbilityEditor } from '../components/AbilityEditor';
import { EntityInflict, EntityAbility } from '../../engine/EntityStats';
...
class EntityManagerTool implements EditorTool {
  ...
  private rebuildForm(): void {
    ...
    this.buildAfflictionSection();

    this.refreshColSection();
  }

  /** Status inflicts (melee) + PSI abilities (ranged casts) for this entity. Most
   *  are CANON-derived by tools/extract_enemy_afflictions.py, but the inflict list
   *  is hand-tunable here; abilities are shown read-only (edit via the tool / JSON). */
  private buildAfflictionSection(): void {
    if (!this.formEl) return;
    const cur = this.entities[String(this.sprite)] ?? {};

    // --- STATUS INFLICTS (melee, editable) ---
    const inflictWrap = buildInflictEditor({
      entries: (cur.inflict ?? []) as InflictEntry[],
      onCommit: (list) => this.setInflict(this.sprite, list),
      emptyText: 'none — this entity inflicts no status on a melee hit',
      addChance: 25,
      noun: 'enemy',
    });
    this.formEl.appendChild(inflictWrap);

    // --- PSI ABILITIES (ranged casts, add/edit by mode) ---
    const abilityWrap = buildAbilityEditor({
      entries: (cur.abilities ?? []) as EntityAbility[],
      onCommit: (list) => this.setAbilities(this.sprite, list),
    });
    this.formEl.appendChild(abilityWrap);
    const hint = document.createElement('div');
    hint.textContent = 'Most PSI is canon-generated by tools/extract_enemy_afflictions.py.';
    hint.style.cssText = 'color:#667;font-size:10px;line-height:1.4;';
    this.formEl.appendChild(hint);
  }

  /** Persist the status-inflict list for a sprite (drops it when empty). */
  private setInflict(sprite: number, list: InflictEntry[]): void {
    const k = String(sprite);
    const next: EntityStats = { ...(this.entities[k] ?? {}) };
    if (list.length) next.inflict = list as EntityInflict[];
    else delete next.inflict;
    if (Object.keys(next).length) this.entities[k] = next;
    else delete this.entities[k];
    this.shell?.markDirty('entities');
  }

  /** Persist the PSI ability list for a sprite (drops it when empty). */
  private setAbilities(sprite: number, list: EntityAbility[]): void {
    const k = String(sprite);
    const next: EntityStats = { ...(this.entities[k] ?? {}) };
    if (list.length) next.abilities = list;
