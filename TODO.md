...
- [x] PK player toggle — **PK** item in the command menu opens a confirm ("anyone can kill you, can't turn it off for 5 minutes"). Server-authoritative + **persisted** (`pk`/`pkUntil` in the save): enabling arms a 5-min lock that survives relogging; disabling is refused until it expires. Broadcast as `player_pk`, red nameplate marks PKers.
